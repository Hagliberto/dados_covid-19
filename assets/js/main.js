...
//Função para ordernar e popular o combo
...
//Função para popular os KPI (indicadores) globais
function loadSummary(data) {
    ...
    active.innerText = formateDate(new Date(data.Global.Date));
...
function loadData(data) {
    ...
    insertDailyData("tconfirmed", tConfirmedDelta, yConfirmedDelta < tConfirmedDelta)
    insertDailyData("tdeath", tDeathDelta, yDeathDelta < tDeathDelta)
    insertDailyData("trecovered", tRecoveredDelta, yRecoveredDelta < tRecoveredDelta )
...
function insertDailyData(element, value, increase){
    if(increase){
        document.getElementById(element).innerHTML = `<img src= '.../up.png'> Diário ${value.toLocaleString("PT")}`
    } else {
        document.getElementById(element).innerHTML = `<img src= '.../down.png'> Diário ${value.toLocaleString("PT")}`
    }
}
